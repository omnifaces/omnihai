 ...
 * <p>
 * The tool name is derived from the method signature, so {@code findOrder} of {@code OrderTools} becomes {@code OrderTools_findOrder}, and overloads of it
 * carry their parameter types as {@code OrderTools_findOrder_long} and {@code OrderTools_findOrder_String}. The description is what the AI reads when deciding
 * whether the tool applies, so state what it returns and when it is appropriate.
 * <p>
 * Usage example:
 *
 * <pre>
